...
export async function GET(request: Request, { params }: RouteParams) {
  ...
  if (format === "png") {
    const png = await buildStudentCardPng("back", renderData);
    return new NextResponse(new Uint8Array(png), {
      headers: {
        "Content-Type": "image/png",
        "Cache-Control": "no-store",
        "Content-Disposition": `${download ? "attachment" : "inline"}; filename="${fileBase}-verso.png"`,
      },
    });
  }

  const svg = await buildStudentCardSvg("back", renderData);

  return new NextResponse(svg, {
    headers: {
      "Content-Type": "image/svg+xml; charset=utf-8",
      "Cache-Control": "no-store",
      "Content-Disposition": `${download ? "attachment" : "inline"}; filename="${fileBase}-verso.svg"`,
    },
  });
}
